...
public class ControladorTelaGruposGerenAtendimentos extends ControladorTela{
    ...
    public void gerarAtendimento() {
        try{
            if (estaComItemValidoSelecionado(jcbModuloAtividade)) {
                ...
                EspecificacaoGeracaoAtendimentoDTO especificacaoGeracaoAtendimentosDTO = new EspecificacaoGeracaoAtendimentoDTO(moduloPeriodoDTO.getId(), "", null);
                ResultadoGeracaoAtendimentoDTO resultadoGeracaoAtendimentoDto = servicoSisLaraServer.gerarAtendimentos(especificacaoGeracaoAtendimentosDTO, Sessao.obterInstancia().obterToken());
                new TelaGruposEditarAtendimentos((JDialog) telaPai, resultadoGeracaoAtendimentoDto.getGrupoDto(), resultadoGeracaoAtendimentoDto.getModuloPeriodoDTO(), (AtendimentoGrupoDTO)resultadoGeracaoAtendimentoDto.obterObjetoDtoEditado(), false);
            ...
            JOptionPanePersonalizado.mostrarTelaErro(telaPai, JOptionPanePersonalizado.ERRO_GERACAO_MODELO_ATENDIMENTO);
            logger.fatal(JOptionPanePersonalizado.ERRO_GERACAO_MODELO_ATENDIMENTO + e);
        }
    }
    ...
    private void inserirAtendimentoNoModuloAtividadeSelecionado(){
        ...
        if (Sessao.obterInstancia().possuiDto(chave)){
            adicionarDtos((AtendimentoGrupoDTO) Sessao.obterInstancia().obterDto(chave), jtaListagemAtendimentos);
        }    
    }
