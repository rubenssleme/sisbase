...
import br.laramara.ti.sislaraclient.telas.TelaRelatorioFrequenciaDeAtendimentosGlobaisPorUsuariosNoPeriodo;
import br.laramara.ti.sislaraclient.telas.TelaRelatorioPorcentagensFrequenciaPorGrupo;
...
public class ControladorTelaPrincipal extends ControladorTela {
    ...
    public void resolverPendencia() {
        if (estaComItemValidoSelecionado(jliPendencias)) {
            ...
            if (pendenciaDto.ePendenciaAtendimentoIndividual()) {
                    ...
                    public void run() {
                        try {
                            ...
                            if (resultadoGeracaoAtendimentoDTO.sucesso()) {
                                new TelaGruposEditarAtendimentos(telaPai, resultadoGeracaoAtendimentoDTO.getGrupoDto(), resultadoGeracaoAtendimentoDTO.getModuloPeriodoDTO(), (AtendimentoGrupoDTO) resultadoGeracaoAtendimentoDTO.obterObjetoDtoEditado(), true);
                ...
                new EsperaUtils(comando, (JFrame)telaPai).execute();
            }
        } else {
    ...
    public void exiterTelaRelatorioFrequenciaDeAtendimentosGlobaisPorUsuariosNoPeriodo() {
        new TelaRelatorioFrequenciaDeAtendimentosGlobaisPorUsuariosNoPeriodo((JFrame)telaPai);
    }

    public void exibirTelaRelatorioPorcentagensFrequenciaPorGrupo() {
        new TelaRelatorioPorcentagensFrequenciaPorGrupo((JFrame) telaPai);
    }
}
