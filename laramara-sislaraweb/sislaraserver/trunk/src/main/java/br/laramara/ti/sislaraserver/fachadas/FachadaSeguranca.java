...
import br.laramara.ti.sislaraserver.fabricas.FabricaCredencialExterna;
...
public class FachadaSeguranca extends Fachada {
	...
	public ResultadoAutenticacaoDTO autenticarLogin(CredencialDTO credencialDto) {
		...
		Credencial credencial = new FabricaCredencial().converterParaDominio(credencialDto);
		try {
	...
	public ResultadoAutenticacaoDTO autenticarUsuarioExterno(CredencialExternaDTO credencialExternaDto) {
		...
		CredencialExterna credencialExterna = new FabricaCredencialExterna().converterParaDominio(credencialExternaDto);
		try {
			String token = autenticador.autenticaUsuarioExterno(credencialExterna);
			resultado.efetuadoComSucesso(new TokenDTO(token));
		} catch (UsuarioExternoNaoAutenticadoException e) {
			resultado.adicionarErro(e.getMessage());
		} catch (Exception e) {
			logger.error("Ocorreu um erro durante a solicitação de autenticação de usuário externo. \nDetalhes: " + e);
		}
		return resultado;
	}
	...
	public boolean possuiAutorizacao(TokenDTO tokenDto, String funcionalidade) {
		...
		Permissao permissao = Permissao.valueOf(Permissao.class, funcionalidade);
	...
	private boolean estaBloqueadoParaEdicao(String chave, Bloqueador bloqueador, TokenDTO tokenDto) {
		...
		if (chave != null) {
			ContaAcesso contaAcessoEditando = obterBloqueador(chave, bloqueador);
			if (contaAcessoEditando != null && contaAcessoEditando.equals(contaAcessoSolicitouEdicao)) {
				retorno = false;
			} else {
	...
	public ResultadoCoordenacaoEdicaoDTO obterContaAcessoEditando(String nome) {
		...
		ContaAcesso contaAcesso = obterBloqueador(nome, coordenadorEdicaoGeral);

		ContaAcessoDTO contaAcessoDto = new FabricaContaAcesso().converterParaDTO(contaAcesso);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private ContaAcesso obterBloqueador(String chaveBloqueio, Bloqueador bloqueador) {
	...
	public ResultadoListagemGeralBloqueadosDTO obterListagemBloqueados() {
		...
		try {
			Map<String, ContaAcesso> geralContaAcessoBloqueados = coordenadorEdicaoGeral.obterObjetosBloqueados();
			...
			for (String chaveBloqueio : geralContaAcessoBloqueados.keySet()) {
				listagemGeralContaAcessoBloequadoDTO.add(new GeralContaAcessoBloqueadoDTO(chaveBloqueio.toString(),
						new FabricaContaAcesso().converterParaDTO(geralContaAcessoBloqueados.get(chaveBloqueio))));
			}

			resultadoListagemBloqueadosDTO.efetuadoComSucesso(listagemGeralContaAcessoBloequadoDTO);
		} catch (Exception e) {
	...
	public ResultadoAlteracaoExtensaoRelatorioDTO alterarExtensaoRelatorioParaXLS(TokenDTO tokenDto) {
	...
	public ResultadoAlteracaoExtensaoRelatorioDTO alterarExtensaoRelatorioParaPDF(TokenDTO tokenDto) {
		ResultadoAlteracaoExtensaoRelatorioDTO resultadoAlteracaoExtensaoRelatorioDTO = new ResultadoAlteracaoExtensaoRelatorioDTO();
		ContaAcesso contaAcesso = obterContaAcesso(tokenDto);
		try {
	...
	public ResultadoAlteracaoSenhaDTO alterarSenha(CredencialDTO credencialDto, TokenDTO tokenDto) {
		ResultadoAlteracaoSenhaDTO resultadoAlteracaoSenhaDto = new ResultadoAlteracaoSenhaDTO();

		Credencial credencial = new FabricaCredencial().converterParaDominio(credencialDto);
		ContaAcesso contaAcesso = obterContaAcesso(tokenDto);
		if (credencial.novaSenhaValida()) {
			contaAcesso.setSenha(credencial.getSenha());
			repositorioContaAcesso.salvar(contaAcesso);
	...
	public ResultadoListagemContaAcessoDTO pesquisarContaAcessoPor(EspecificacaoPesquisaContaAcessoDTO especificacao) {
		...
		try {
			...
			gravarResultadoSeNaoForVazio(contaAcesso, new FabricaContaAcesso(), resultado);
		} catch (Exception e) {
	...
	public ResultadoListagemPerfilDTO obterListagemPerfil() {
		return (ResultadoListagemPerfilDTO) obterListagem(repositorioPerfil.obterTodos(), new FabricaPerfil(), "Perfil",
				new ResultadoListagemPerfilDTO());
	}

	public ResultadoEdicaoContaAcessoDTO editarContaAcesso(ContaAcessoDTO contaAcessoDto, TokenDTO tokenDto) {
		OperacaoFachada operacao = new OperacaoContaAcessoEdicao(new FabricaContaAcesso(), repositorioContaAcesso,
				contaAcessoDto);
		return (ResultadoEdicaoContaAcessoDTO) verificarPermissaoEProcessar(operacao, Permissao.CONTA_ACESSO_EDICAO,
				new ResultadoEdicaoContaAcessoDTO(), tokenDto);
	}

	public ResultadoListagemPerfilDTO pesquisarPerfilPor(EspecificacaoPesquisaPerfilDTO especificacao) {
	...
	public ResultadoListagemPermissaoDTO obterListagemPermissao() {
		return (ResultadoListagemPermissaoDTO) obterListagem(Arrays.asList(Permissao.values()), new FabricaPermissao(),
	...
	public ResultadoEdicaoPerfilDTO editarPerfil(PerfilDTO perfilDto, TokenDTO tokenDto) {
		OperacaoFachada operacao = new OperacaoPerfilEdicao(repositorioPerfil, perfilDto);
		return (ResultadoEdicaoPerfilDTO) verificarPermissaoEProcessar(operacao, Permissao.PERFIL_EDICAO,
	...
	public ResultadoOperacaoFiltroGrupoDTO desativarFiltroGrupo(TokenDTO tokenDto) {
		ResultadoOperacaoFiltroGrupoDTO resultadoOperacaoFiltroGrupoDTO = new ResultadoOperacaoFiltroGrupoDTO();
		try {
			ContaAcesso contaAcesso = obterContaAcesso(tokenDto);
	...
	public ResultadoListagemPermissaoDTO obterPermissoes(TokenDTO tokenDto) {
		ContaAcesso contaAcesso = repositorioContaAcesso.obterPorToken(tokenDto.getToken());
		List<PermissaoDTO> permissoes = new FabricaPermissao()
				.converterParaDTO(contaAcesso.obterTodasPermissoes());
	...
	public void gravarTela(byte[] tela) {
		try {
			File imagem = new File(new Configuracao().obterConfiguracao(Configuracao.DIRETORIO_TELAS_CAPTURADAS)
					+ DataHoraUtils.obterDataHoraMinutosMilesegundosAtual() + ".jpg");

			FileUtils.writeByteArrayToFile(imagem, tela);
		} catch (Exception ex) {
			logger.fatal("Erro durante armazenamento de imagem de tela. Detalhes: " + ex);
		}
	}

	public ResultadoSolicitacaoRecuperacaoSenhaDTO efetuarSolicitacaoRecuperacaoSenha(String emailSolicitante) {
		...
		try {
			...
			if (usuarioExterno != null) {
				if (recuperadorSenha.enviarEmailDeRecuperacaoSenha(emailSolicitante)) {
					resultadoSolicitacaoRecuperacaoSenhaDTO.efetuadoComSucesso();
				} else {
					resultadoSolicitacaoRecuperacaoSenhaDTO
							.adicionarErro("Erro durante solicitação de alteração de senha.");
				}
			} else {
				resultadoSolicitacaoRecuperacaoSenhaDTO.adicionarErro("E-mail ou CPF não encontrado ou inválido.");
			}
		} catch (Exception e) {
			logger.fatal("Erro durante solicitação de recuperação de senha. Detalhes: " + e.getMessage());
		}

		return resultadoSolicitacaoRecuperacaoSenhaDTO;
	}
	...
	public ResultadoSolicitacaoCadastroNovaSenhaDTO cadastrarNovaSenha(
		...
		try {
			...
			if (solicitacaoAlteracaoSenha.novaSenhaValida()) {
				if (recuperadorSenha.cadastrarNovaSenha(solicitacaoAlteracaoSenha.getChave(),
						solicitacaoAlteracaoSenha.getNovaSenha())) {
					resultadoSolicitacaoCadastroNovaSenhaDTO.efetuadoComSucesso();
				} else {
					resultadoSolicitacaoCadastroNovaSenhaDTO.adicionarErro("Erro durante cadastro de nova senha.");
				}
			} else {
				resultadoSolicitacaoCadastroNovaSenhaDTO
						.adicionarErro("Erro durante cadastro de nova senha. As senhas não estão iguais.");
			}
		} catch (Exception e) {
			String erro = "Erro durante cadastro de nova senha.";
			resultadoSolicitacaoCadastroNovaSenhaDTO.adicionarErro(erro);
			logger.fatal(erro + " Detalhes: " + e.getMessage());
		}

		return resultadoSolicitacaoCadastroNovaSenhaDTO;
	}

	public boolean tokenAtivo(TokenDTO tokenDTO) {
		return autenticador.tokenAtivo(tokenDTO);
	}
}
