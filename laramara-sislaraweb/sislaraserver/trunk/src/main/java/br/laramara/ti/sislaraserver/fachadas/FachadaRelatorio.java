...
public class FachadaRelatorio extends Fachada {
	...
	public ArquivoDTO gerarRelatorioAvaliacaoPsicossocial(Long prontuario, TokenDTO tokenDto) {
		return gerarRelatorio(Permissao.RELATORIO_AVALIACAO_PSICOSSOCIAL,
				ModeloRelatorio.AVALIACAO_PSICOSSOCIAL, gerarParametroProntuarioEFoto(prontuario),
				tokenDto);
	}
	...
	private Map<String, Object> gerarParametroProntuarioEFoto(Long prontuario) {
		Map<String, Object> parametros = gerarParametroProntuario(prontuario);
		try {
			parametros.put("param_foto", new FileInputStream(ArquivoUtils.obterArquivoFoto(prontuario)));
		} catch (Exception e) {
			logger.fatal("Erro durante passagem foto. Detalhe: " + e);
		}
		return parametros;
	}
