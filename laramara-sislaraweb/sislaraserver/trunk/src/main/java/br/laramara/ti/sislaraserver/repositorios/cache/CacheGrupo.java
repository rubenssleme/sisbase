...
public class CacheGrupo {
	...
	public List<Grupo> obterAtivosComModuloPeriodoAEE() {
		return obterAtivos().stream().filter(grupo -> grupo.possuiModuloPeriodoAEE()).collect(Collectors.toList());
	}
	...
	public Grupo obterGrupoAtivoEPublicadoComDescricaoTipoAtendimentoDeCursosPorIdGrupo(Long idGrupo) {
		return obterGruposAtivosEPublicadosComDescricaoTipoAtendimentoDeCursos().stream()
				.filter(grupo -> grupo.getId().equals(idGrupo)).findFirst().orElse(null);
	}
	
	public List<Grupo> obterGruposAtivosEPublicadosComDescricaoTipoAtendimentoDeCursos() {
		return obterAtivos().stream().filter(grupo -> grupo.eDescricaoCursos() && grupo.isPublicado())
	...
	public List<Profissional> obterProfissionaisDeModulosPeriodosDeGruposAtivosComUsuarioIntegrado(Usuario usuario) {
		List<ModuloPeriodo> modulosPeriodosDeGruposAtivosComUsuarioIntegrado = obterModuloPeriodoDeGruposAtivosComUsuarioIntegrado(usuario);
	...
	private List<ModuloPeriodo> obterModuloPeriodoDeGruposAtivosComUsuarioIntegrado(Usuario usuario) {
		return (List<ModuloPeriodo>) obterModulosPeriodosDeGruposAtivosComUsuarioNaoDesligado(
				usuario).stream()
						.filter(moduloPeriodo -> moduloPeriodo.existeUsuarioIntegrado(usuario)
								...
								&& !moduloPeriodo.ePossuiModuloDeReuniaoComInstituicaoDeEnsino()
						&& !obterGrupoPorIdModuloPeriodo(moduloPeriodo.getId()).eDescricaoOrientacaoSocioeducativa())
						.collect(Collectors.toList());
	}

	public List<ModuloPeriodo> obterModuloPeriodoServicoSocialReuniaoIntegracaoAtivosComDataFutura(
		...
		List<ModuloPeriodo> moduloPeriodoDeGrupoDeServicoSocialComModuloReuniaoDeIntegracao = obterAtivos().stream()
				.filter(grupo -> grupo.isAtivo() && grupo
						.eDescricaoServicoSocialModuloReuniaoDeIntegracaoComDataFutura(moduloPeriodo))
	...
	public Grupo obterGrupoAtivoMaisRecenteDeReuniaoDeIntegracaoComUsuarioIntegrado(Usuario usuario) {
		Grupo grupoAtivoMaisRecenteDeReuniaoDeIntegracaoComUsuarioIntegrado = obterAtivos().stream()
				.filter(grupo -> grupo.isAtivo() && grupo.existeUsuarioIntegradoEmModuloDeReunicaoDeIntegracao(usuario))
				.sorted(Grupo.obterComparadorDataInicio().reversed()).findFirst().orElse(null);
		return grupoAtivoMaisRecenteDeReuniaoDeIntegracaoComUsuarioIntegrado;
	}
	...
	public List<Grupo> obterGruposDescricaoServicoSocialModuloReuniaoDeIntegracaoAtivos(
		...
		List<Grupo> gruposDescricaoServicoSocialComModuloReuniaoDeIntegracaoAtivos = obterAtivos().stream()
				.filter(grupo -> grupo.isAtivo() && grupo.possuiDescricaoEModulo(descricaoTipoAtendimento, modulo))
				.collect(Collectors.toList());
		return new Cloner().deepClone(gruposDescricaoServicoSocialComModuloReuniaoDeIntegracaoAtivos);
	}

	public ModuloPeriodo obterModuloPeriodoPorAtendimentoGrupo(AtendimentoGrupo atendimentoGrupo) {
		inicializar();
		Grupo grupo = obterGrupoPorIdAtendimentoGrupo(atendimentoGrupo.getId());
		return grupo.getModulosPeriodos().stream()
				.filter(moduloPeriodo -> moduloPeriodo.getAtendimentosGrupo().contains(atendimentoGrupo)).findFirst()
				.orElse(null);
	}
