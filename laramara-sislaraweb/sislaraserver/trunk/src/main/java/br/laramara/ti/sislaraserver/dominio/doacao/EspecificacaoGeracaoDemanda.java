	...
	private void validarObrigatoriedadeDeDadosDoUsuarioInterno() {
		if (possuiUsuario()){
			if (!usuario.possuiDataNascimento()){
				adicionarErro("Insira a data de nascimento do usuário.");
			}
			if (!usuario.possuiCpf()){
				adicionarErro("Insira o CPF do usuário.");
			}
			if (usuario.naoPossuiRg()){
				adicionarErro("Insira o RG do usuário.");
			}
			if (!usuario.possuiEmail()){
				adicionarErro("Insira o email do usuário.");
			}
			if (!usuario.possuiEndereco()){
				adicionarErro("Insira o endereço do usuário.");
			}
			if (usuario.eMenorIdadeENaoPossuiFamiliarPrincipalResponsavelComCPFRGeEmail()){
				adicionarErro("Insira um familiar que é principal responsável com CPF, RG e Email.");
			}
		}
	}
