...
import br.laramara.ti.sislaraserver.fabricas.FabricaOrigemEncaminhamentoDetalhado;
...
import br.laramara.ti.sislaraserver.fabricas.FabricaVulnerabilidade;
import br.laramara.ti.sislaraserver.fabricas.externa.FabricaDescricaoEvento;
import br.laramara.ti.sislaraserver.fabricas.externa.FabricaTipoDescricaoEvento;
...
public class FachadaGrupo extends Fachada {
	...
	@Inject
	private RepositorioVulnerabilidade repositorioVulnerabilidade;
	@Inject
	private GerenciadorGrupo gerenciadorGrupo;
	
	public ResultadoListagemTipoDescricaoEventoDTO obterListagemTipoDescricaoEvento() {
		return (ResultadoListagemTipoDescricaoEventoDTO) obterListagem(
				Arrays.asList(TipoDescricaoEvento.values()), new FabricaTipoDescricaoEvento(),
				"Tipo da Descrição do Evento", new ResultadoListagemTipoDescricaoEventoDTO());
	}
	...
	public ResultadoConsultaDetalheCursoDTO obterDetalheCursoPorIdGrupo(Long idGrupo) {
		...
		if (!NumeroUtils.numeroLongoInvalido(idGrupo)) {
			Grupo grupoAtivoComDescricaoTipoAtendimentoDeCursos = repositorioGrupo
					.obterGrupoAtivoEPublicadoComDescricaoTipoAtendimentoDeCursosPorIdGrupo(idGrupo);

			if (grupoAtivoComDescricaoTipoAtendimentoDeCursos != null) {
				resultadoConsultaDetalheCursoDTO.efetuadoComSucesso(
						new FabricaDetalheCurso().converterParaDTO(grupoAtivoComDescricaoTipoAtendimentoDeCursos));
			} else {
				resultadoConsultaDetalheCursoDTO.adicionarErro("Nenhum detalhe de curso encontrado.");
			}
		} else {
			resultadoConsultaDetalheCursoDTO.adicionarErro("Nenhum detalhe de curso encontrado.");
		}

		return resultadoConsultaDetalheCursoDTO;
	}
	
	public ResultadoListagemDetalhesCursoDTO obterListagemDetalhesCursos() {
		return (ResultadoListagemDetalhesCursoDTO) obterListagem(
				repositorioGrupo.obterGruposAtivosEPublicadosComDescricaoTipoAtendimentoDeCursos(), new FabricaDetalheCurso(),
				"Detalhes de Cursos", new ResultadoListagemDetalhesCursoDTO());
	}
	...
	public ResultadoListagemRecursoDTO obterRecursosDisponiveisParaDemanda() {
		return (ResultadoListagemRecursoDTO) obterListagem(
				repositorioRecurso.obterDisponivelParaDemanda(), new FabricaRecurso(),
				"Recursos disponível para demanda", new ResultadoListagemRecursoDTO());
	}
	...
	public ResultadoValidacaoDescricaoEventoDTO validarDescricaoEvento(DescricaoEventoDTO descricaoEventoDto) {
		return (ResultadoValidacaoDescricaoEventoDTO) Validador.efetuarValidacao(
				descricaoEventoDto, new FabricaDescricaoEvento(),
				"Curso / Evento", new ResultadoValidacaoDescricaoEventoDTO());
	}
	...
	public ResultadoValidacaoLoteRecursoDTO validarLoteRecurso(
		...
		return (ResultadoValidacaoLoteRecursoDTO) Validador.efetuarValidacao(
				loteRecursoDto, new FabricaLoteRecurso(), "Lote Recurso",
		...
		if (contaAcesso.possuiPermissao(Permissao.GRUPO_EDICAO)) {
			if (repositorioGrupo.confirmaVersaoAtualPorIdModuloPeriodo(
				...
				ModuloPeriodo moduloPeriodo = new FabricaModuloPeriodo().converterParaDominio(moduloPeriodoDto,
						repositorioGrupo.obterModuloPeriodoPorId(moduloPeriodoDto.getId()));
	...
	public ResultadoValidacaoEncaminhamentoDTO validarOrigemEncaminhamentoDetalhado(OrigemEncaminhamentoDetalhadoDTO encaminhamentoDto) {
		return (ResultadoValidacaoEncaminhamentoDTO) Validador.efetuarValidacao(encaminhamentoDto,
				new FabricaOrigemEncaminhamentoDetalhado(), "Origem Encaminhamento Detalhado", new ResultadoValidacaoEncaminhamentoDTO());
	}

	public ResultadoListagemVulnerabilidadeDTO obterListagemVulnerabilidadeParaUsuario() {
		return (ResultadoListagemVulnerabilidadeDTO) obterListagem(repositorioVulnerabilidade.obterTodosParaUsuario(),
				new FabricaVulnerabilidade(), "Vulnerabilidade disponivel para Usuario",
				new ResultadoListagemVulnerabilidadeDTO());
	}
	
	public ResultadoListagemVulnerabilidadeDTO obterListagemVulnerabilidadeParaFamilia() {
		return (ResultadoListagemVulnerabilidadeDTO) obterListagem(repositorioVulnerabilidade.obterTodosParaFamilia(),
				new FabricaVulnerabilidade(), "Vulnerabilidade disponivel para Familia",
				new ResultadoListagemVulnerabilidadeDTO());
	}
}
