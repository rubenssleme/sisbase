...
public class RepositorioRecursoBD extends
	...
	public List<Recurso> obterDisponivelParaDemanda() {
		...
		try {
			TypedQuery<Recurso> query = em.createQuery(
					"SELECT i FROM Recurso i WHERE i.disponivelParaDemanda is TRUE ORDER BY i.descricao",
					Recurso.class);
			recurso = query.getResultList();
		} catch (Exception e) {
			logger.fatal("Erro durante pesquisa de Recursos disponíveis para demanda.\n Detalhes:"
					+ e);
		}
		return recurso;
	}
