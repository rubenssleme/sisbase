...
public class TestesAgendamento {
	...
	@Test(groups = { TiposDeTeste.UNITARIO })
	public void agendamento_com_erro_de_tamanho_maximo_de_dados() {

		Agendamento agendamento = ContextoAgendamento
				.fabricarAgendamentoComTodosOsDados();
		agendamento.setJustificativaCancelamento(ContextoGenerico.obterGrande());
		agendamento.setInformacaoAdicional(ContextoGenerico.obterGrande());
		agendamento.validarObrigatoriedadeETamanhoMaximoDeDados();

		Assert.assertFalse(agendamento.validado());
		Assert.assertTrue(agendamento.obterDescricaoErros().contains("Insira a Justificativa de cancelamento contendo até 20000 caracteres."));
		Assert.assertTrue(agendamento.obterDescricaoErros().contains("Insira a Informação adicional contendo até 20000 caracteres."));
	}
