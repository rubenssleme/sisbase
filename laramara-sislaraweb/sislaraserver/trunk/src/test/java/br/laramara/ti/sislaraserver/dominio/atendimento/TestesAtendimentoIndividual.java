...
public class TestesAtendimentoIndividual {
	...
	public void atendimento_individual_construido_com_sucesso() {
		...
		InformacaoAtendimento informacaoAtendimento = ContextoInformacaoAtendimento
				.fabricarInformacaoAtendimentoATComTodosOsDados();
		...
		atendimentosProfissional.add(ContextoAtendimentoProfissional.fabricarAtendimentoProfissionalComATComTodosOsDados());
		...
		Assert.assertEquals(atendimento.getHorario().toString(),
				horario.toString());
		Assert.assertFalse(atendimento.getInformacaoAtendimento().getParticipacaoDetalhada().isEmpty());
	}
	...
	public void atendimento_individual_validacao_com_erro_de_obrigatoriedade_primeira_vez_ou_retorno() {
		...
		Assert.assertFalse(atendimento.validado());
		Assert.assertEquals(atendimento.obterNumeroErros(), 9);
	}
	
	@Test(groups = { TiposDeTeste.UNITARIO })
	public void atendimento_individual_validacao_sem_erro_contendo_usuario_com_frequencia_diferente_de_AT_sem_participacao_detalhada() {
		AtendimentoIndividual atendimento = ContextoAtendimentoIndividual.fabricarComTodosOsDados();
		atendimento.getInformacaoAtendimento().setFrequencia(Frequencia.FJ);
