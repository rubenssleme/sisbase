...
public class TestesUsuario {
	...
	public void usuario_validacao_com_erro_de_tamanho_de_dados() {
		...
		usuario.setCirurgias(textoGrande);
		usuario.setMedicamentos(textoGrande);
		usuario.setConsanguinidade(textoGrande);
		usuario.setHistorico(textoGrande);
		usuario.setFuncionalidade(textoGrande);
		usuario.setReacaoFrenteADeficiencia(textoGrande);
		usuario.setReacaoFrenteADeficienciaFamiliar(textoGrande);
		usuario.setRedeDeApoio(textoGrande);
		usuario.setRedeDeAmigos(textoGrande);
		usuario.setNamoroCasamentoSexualidade(textoGrande);
		usuario.setNecessidadesExpectativasQueixas(textoGrande);
		usuario.setEducacional(textoGrande);
		usuario.setComunicacao(textoGrande);
		usuario.setReligiaoLazerCulturaRotina(textoGrande);
		usuario.setParecer(textoGrande);
		...
		usuario.setOrigemEncaminhamentosDetalhados(Arrays.asList(encaminhamento));

		usuario.validarObrigatoriedadeETamanhoMaximoDeDados();

		Assert.assertFalse(usuario.validado());
		Assert.assertNotNull(usuario.obterDescricaoErros());
		Assert.assertEquals(usuario.obterNumeroErros(), 36,
				"Deveriam haver 36 infrações de largura máxima de dados.");
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Exite um encaminhamento inválido."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira uma Renda válida."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira uma Data de Nascimento válida."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Ramal contendo até 6 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Nome de Contato contendo até 100 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Email contendo até 100 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Nome contendo até 100 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um RG contendo até 15 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Endereço contendo até 60 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Número contendo até 10 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Complemento contendo até 50 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Bairro contendo até 50 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira a Naturalidade contendo até 30 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira a Nacionalidade contendo até 30 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira o Orgão Emissor contendo até 25 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira o Outros Apoios/Serviços contendo até 100 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira uma Obs contendo até 20000 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira um Tamanho de Fonte contendo até 100 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira as Cirurgias contendo até 20000 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira as Medicamentos contendo até 20000 caracteres."));
		Assert.assertTrue(usuario.obterDescricaoErros().contains("Insira as Consanguinidade contendo até 20000 caracteres."));
	}
	...
	public void usuario_adiciona_historico_status_com_sucesso() {
		StatusRelacaoComModulo status = StatusRelacaoComModulo.CASO_NOVO;

		Usuario usuario = ContextoUsuario.fabricarUsuarioComTodosOsDados();
		usuario.adicionarStatusUsuario(status, "TESTE");

		Assert.assertEquals(usuario.obterHistoricoStatusUsuarioAtual().getStatus(),
				status);
	}
	...
	public void usuario_atualiza_status_de_retorno_cto_com_sucesso() {
		...
		usuario.adicionarStatusUsuario(StatusRelacaoComModulo.DESISTENTE, "TESTE");
