...
public class OperacaoAgendamentoEdicao implements OperacaoFachada  {
	...
	public ResultadoDTO processar(ContaAcesso contaAcesso,
		...
		agendamento.setContaAcessoResponsavelPorOperacao(contaAcesso);
	...
	private void salvarPendenciaAtendimentoIndividual(Agendamento agendamentoSalvo) {
		...
		pendencia.setProfissionaisAfetados(Arrays.asList(agendamentoSalvo.getProfissional()));
		pendencia.setAgendamento(agendamentoSalvo);
		if (agendamentoSalvo.possuiUsuario()) {
			pendencia.setProntuario(agendamentoSalvo.getUsuario().getId());
			pendencia.setTipoPendencia(TipoPendencia.ATENDIMENTO_INDIVIDUAL_USUARIO);
		} else if (agendamentoSalvo.possuiPreCadastro()) {
			pendencia.setTipoPendencia(TipoPendencia.ATENDIMENTO_INDIVIDUAL_PRE_CADASTRO);
		}
