	...
	private void salvarPendenciaAtendimentoGrupo(Grupo grupo) {
		if (grupo.isAtivo() && (grupo.podeCriarPendenciaDeAtendimento())) {
			for (ModuloPeriodo moduloPeriodo : grupo.getModulosPeriodos()) {
				if (!moduloPeriodo.ePsicossocialParaAsFamiliasOuPsicossocialParaOsJovensOuReuniaoComAsInstituicoesDeEnsino()) {
					for (DiaSemanaEHorario diaSemanaEHorario : moduloPeriodo.getDiasSemanaEHorariosDaAtividade()) {
						...
						for (Calendar dataDoDiaDaSemanaNoPeriodo : datasDoDiaDaSemanaNoPeriodo) {
							...
							pendencia.setData(dataDoDiaDaSemanaNoPeriodo);
							pendencia.setHorario(diaSemanaEHorario.getHorario());
							pendencia.setTipoPendencia(TipoPendencia.ATENDIMENTO_GRUPO);
							pendencia.setProfissionaisAfetados(moduloPeriodo.obterSomenteProfissionais());
							pendencia.setGrupo(grupo);
							pendencia.setModuloPeriodo(moduloPeriodo);
