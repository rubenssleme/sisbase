...
public class AutomatizadorEspera extends Automatizador {
	...
	private void salvarPendenciaEnvioDeEsperaServioSocialPorExcessoDeFaltas(Espera espera, List<Profissional> profissionaisAfetados) {
		...
		pendencia.setProntuario(espera.getUsuario().getId());
		pendencia.setTipoPendencia(TipoPendencia.ENVIO_LISTA_ESPERA_EXCESSO_DE_FALTAS);
		pendencia.setProfissionaisAfetados(profissionaisAfetados);
	...
	public Resultado processarAtendimentoGrupoInclusaoNaListaDeEsperaPorExecessoDeFaltas(
		...
		for (AtendimentoUsuario atendimentoUsuario : atendimentosGrupo.getAtendimentosUsuarios()) {
	...
	public void incluirNaListaDeEsperaDeAvaliacaoFuncionalSeNecessario(AtendimentoIndividual atendimentoIndividual) {
		if (atendimentoIndividual.possuiUsuario()) {
			...
			if (atendimentoIndividual.ePrimeiraVez()
					&& quantidadeAtendimentoIndividualComDescricaoAvaliacaoServicoAtencaoEspecializadaOftalmologiaModuloAEEeFrequenciaAT == 1
					&& repositorioEspera.obterPor(especificacaoPesquisaEspera).isEmpty()) {
				Espera espera = new Espera();
				espera.setUsuario(usuario);
				...
				espera.setObs("Incluído automaticamente em consequência ao primeiro atendimento em oftalmologia.");
				espera.atribuirDataAtualNaDataExpectativa();
				espera.setSetor(Setor.NENHUM);
				espera.setTipoEspera(TipoEspera.CN);
				espera.validarObrigatoriedadeETamanhoMaximoDeDados();
