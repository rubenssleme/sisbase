update modulo_usuario set status = 'DESISTENTE' where id in (
	...
	where modusu.id_usuario = 10076 and gru.ativo = true and modusu.status = 'DESLIGADO'
);
...
update modulo_usuario set status = 'DESISTENTE' where id in (
	select modusu.id from modulo_usuario modusu 
		...
		inner join grupo gru
			on gru.id = grumodper.id_grupo
	where modusu.id_usuario = 10183  and gru.ativo = true and modusu.status = 'DESLIGADO'
);
