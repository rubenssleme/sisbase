...
public abstract class ControladorTelaEditar extends ControladorTela implements SessaoUtilizavel, CombosAuxiliaresInicializaveis, CamposDadosCarregavel{
    ...
    protected void adicionarArquivo(JList jliArquivos) {
        ...
        if (opcao == JFileChooser.APPROVE_OPTION){
            try {
                if (jfcArquivo.getSelectedFile() != null) {
                    File arquivoSelecionado = jfcArquivo.getSelectedFile();
                    ArquivoDTO arquivoDTO = new ArquivoDTO(null, arquivoSelecionado.getName(), FileUtils.readFileToByteArray(arquivoSelecionado), null);
