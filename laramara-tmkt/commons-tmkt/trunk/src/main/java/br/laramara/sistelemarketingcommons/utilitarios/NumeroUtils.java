package br.laramara.sistelemarketingcommons.utilitarios;
	...
	public static Integer retornaInteiroOuInvalido(String texto) {
		...
		try {
			if (texto != null && !TextoUtils.estaVazio(texto)) {
				...
				if (((Matcher) padraoApenasNumeros.matcher(TextoUtils.removerCaracteresInvalidos(texto))).matches()) {
					retorno = new Integer(TextoUtils.removerCaracteresInvalidos(texto));
				} else {
